...
$(document).ready(function() {
    ...
    animate: function(time, fps, context) {
      ...
      if (leavingSection) {
        ...
        for (let i = 0; i < points.length; i++) {
          ...
          point.rotate(point);
          drawLine(point);
          if (height * activeSection <= point.y+point.size && point.y-point.size <= height * (activeSection + 1)) {
            drawPoint(point);
          }
        }
      }
  ...
  if (!isMobile) {
    ...
    aroundAnchor.depth = false;
    mousePosition.x = xCenter;
    mousePosition.y = yCenter;
  }
});
